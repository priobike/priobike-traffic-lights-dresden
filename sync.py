...
while True:
    response = requests.get('http://priobike.vkw.tu-dresden.de:20055/FROST-Server/v1.1/Things')
    if len(response.json()['value']) == 0:
        break
    print(f"Deleting {len(response.json()['value'])} things")
    for thing in tqdm(response.json()['value']):
        requests.delete(f'http://priobike.vkw.tu-dresden.de:20055/FROST-Server/v1.1/Things({thing["@iot.id"]})')

with open('sg1.json') as f:
    sg1 = json.load(f)
requests.post('http://priobike.vkw.tu-dresden.de:20055/FROST-Server/v1.1/Things', json=sg1)

with open('sg2.json') as f:
    sg2 = json.load(f)
requests.post('http://priobike.vkw.tu-dresden.de:20055/FROST-Server/v1.1/Things', json=sg2)
...
for i, geometry in tqdm(enumerate(traffic_light_geometries)):
    ...
    dstr_program = {
        ...
        "ObservedProperty": {
            ...
            "@iot.id": dstr_program_observed_property_id, # Completely irrelevant
            "name": "Signal",
    ...
    response = requests.post('http://priobike.vkw.tu-dresden.de:20055/FROST-Server/v1.1/Things', json=sg_json)